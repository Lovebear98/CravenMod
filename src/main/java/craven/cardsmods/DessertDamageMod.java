...
public class DessertDamageMod extends AbstractCardModifier {
    ...
    public static final String ID = makeID(DessertDamageMod.class.getSimpleName());


    @Override
    public float modifyDamage(float damage, DamageInfo.DamageType type, AbstractCard card, AbstractMonster target) {
        if(card instanceof SecondsInterface){
            if(((SecondsInterface) card).SecondsCount() == 0){
                return damage * 3;
            }
        }
