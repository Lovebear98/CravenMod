...
import craven.util.TriFunction;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.Map;

import static craven.util.GeneralUtils.removePrefix;
import static craven.util.TextureLoader.getCardTextureString;


public abstract class BaseCard extends CustomCard {
    final private static Map<String, DynamicVariable> customVars = new HashMap<>();

    protected static String makeID(String name) { return CravenMod.makeID(name); }
    ...
    public void upgrade()
    {
        if (!upgraded)
        {
            ...
            if (this.upgradesDescription)
            {
                ...
                {
                    CravenMod.logger.error("Card " + cardID + " upgrades description and has null upgrade description.");
                }
                else
                {
