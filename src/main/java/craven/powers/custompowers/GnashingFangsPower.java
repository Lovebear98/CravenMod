...
public class GnashingFangsPower extends BasePower implements CloneablePowerInterface {
    ...
    public void atStartOfTurnPostDraw() {
        ...
        addToBot(new AddTemporaryHPAction(owner, owner, Math.max(1, amount)));
        addToBot(new ApplyPowerAction(owner, owner, new RavenousPower(owner, Math.max(1, amount/2))));
    }

    @Override
    public void updateDescription() {
        this.description = DESCRIPTIONS[0] + Math.max(1, amount) + DESCRIPTIONS[1] + Math.max(1, amount/2) + DESCRIPTIONS[2];
    }
    ...
    public AbstractPower makeCopy() {
        return new GnashingFangsPower(owner);
    }
}