...
import coyotle.util.TriFunction;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.Map;

import static coyotle.util.GeneralUtils.removePrefix;
import static coyotle.util.TextureLoader.getCardTextureString;


public abstract class BaseCard extends CustomCard {
    final private static Map<String, DynamicVariable> customVars = new HashMap<>();

    protected static String makeID(String name) { return CoyotleMod.makeID(name); }
    ...
    public void upgrade()
    {
        if (!upgraded)
        {
            ...
            if (this.upgradesDescription)
            {
                ...
                {
                    CoyotleMod.logger.error("Card " + cardID + " upgrades description and has null upgrade description.");
                }
                else
                {
