...
import coyotle.powers.custompowers.TurnEndDiscardHandPower;
...
@NoCompendium
public class EnterTheDream extends AbstractDreamingCard {
    public static final String[] EXTENDED_DESCRIPTION = CardStrings.getMockCardString().EXTENDED_DESCRIPTION;
    public static final String ID = makeID(EnterTheDream.class.getSimpleName());
    ...
    public EnterTheDream(){
        this(true);
    }
    public EnterTheDream(boolean makePreview) {
        super(ID, info);
        ...
        if(makePreview){
            AbstractDreamingCard c = new EnterTheDream(false);
            c.onEnterDream();
    ...
    protected void standardUse(AbstractPlayer p, AbstractMonster m) {
        addToBot(new MillAction(p.drawPile.size()));
    }

    @Override
    protected void dreamUse(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new TurnEndDiscardHandPower()));
    }


    @Override
    public AbstractCard makeCopy() { //Optional
        return new EnterTheDream();
    }
}